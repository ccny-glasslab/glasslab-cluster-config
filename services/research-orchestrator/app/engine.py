...
'citations empty rather than guessing.\n'
    ...
    def _draft_protocol(self, run_id: str, feedback: str | None = None) -> None:
        ...
        prompt = (
            ...
            'remain controlled by the orchestrator. Set evaluation_contract_'
            'proposal.evaluator_type to a NEW task-specific contract id '
            'derived from the objective (e.g. `titanic-survival-methodology-'
            'v1`), never to the referenced contract id above and never to a '
            'generic template name: the referenced contract is a framework '
            'template, and binding the run to it would fail scientific '
            'compatibility. The experiment workload '
    ...
    def _beaker_draft_contract(
        ...
        *,
        ...
        prompt = (
            ...
            'JSON schemas, and concise test vectors or tests. All files in '
            'the candidate directory must use only the suffixes .json, .md, '
            '.py, .txt, .yaml, or .yml (the orchestrator rejects any other '
            'file type). contract.json '
...
'no others: contract_id (string; it must be EXACTLY the approved '
    ...
    def _beaker_implement(self, run_id: str) -> None:
        ...
        prompt = (
            ...
            'of a YAML file you actually wrote in your worktree (the '
            'methodology config holding the contract methodology settings); '
            'the orchestrator rejects a base_config that does not exist. '
            'Matrix seeds create separate cluster jobs. If the workload '
