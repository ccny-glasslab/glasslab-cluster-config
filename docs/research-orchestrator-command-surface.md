...
builds a final acceptance assessment from two recorded turns — the latest
completed Honeydew verification turn and the promoted final-report turn — and
records it as a `final_acceptance.assessment_recorded` event. The assessment
combines findings the agents declared (structured contradictions,
methodological limitations, advisory disagreement) with mechanical citation
checks across both turns' claims: a claim that cites no durable evidence, or
cites an artifact, job, contract component, knowledge record, or context
packet that does not resolve against run state, becomes a `missing_evidence`
finding.

Two boundaries are deliberate:

- `git://` and `event://` citations are **unchecked by v1**: they are listed
  in `unchecked_citations`, which neither satisfies a claim nor marks it
  unresolved. They are not evidence resolution.
- The assessment never analyzes report prose. Prose-level defects (an
  inverted metric comparison, a mischaracterized seed design) require the
  structured claim/evidence layer tracked separately (#213).

When the assessment has unresolved entries, the Discord brief lists them
before anything else variable-length and the generic Approve button is
replaced by an explicit **"Approve & acknowledge N findings"** control;
ordinary approval cannot acknowledge them. HTTP approvers pass
`acknowledge_unresolved_findings: true` plus
`acknowledged_findings_digest` naming the exact assessed state — a stale
digest is rejected. Every acknowledgement is recorded as an idempotent
`action.findings_acknowledged` event carrying the assessment digest, the
count, the reviewer, and every unresolved item with its source turn; the
terminal `run.completed` event carries `findings_acknowledged` plus
`unresolved_findings_count`. Clean assessments keep one-click approval.

If Honeydew rewrites a rejected report, the mechanical assessment is
recomputed for the new report turn and earlier verification findings are
retained with their original turn attribution — but a rewritten report is
not independently re-verified unless a new verification turn actually ran:
verification covers experimental evidence, not prose.
...
that any unresolved verification or final-report-turn findings at acceptance
time were disclosed and explicitly acknowledged — bound to the exact
assessment digest — by the accepting human.

`COMPLETE` does **not** mean the scientific conclusions are flawless,
exhaustively replicated, free of prose errors, or peer-reviewed. Findings
assessment is metadata about what was disclosed, not a validation of the
report's claims. Later readers should inspect
`action.findings_acknowledged`, `final_acceptance.assessment_recorded`, and
the run's artifact digests rather than treating the terminal state as a
review verdict.
